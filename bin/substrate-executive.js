...
const MAP_STATE = join(PROJECT_ROOT, '.ouroboros', 'map_state.json');
const OUTPUT_DIR = join(PROJECT_ROOT, '.ouroboros', 'visualizations');
const NEURAL_PATHS = join(PROJECT_ROOT, '.ouroboros', 'visualizations', 'neural_paths.json');
const RATIFIED_DECISION = join(PROJECT_ROOT, '.ouroboros', 'visualizations', 'ratified_decision.json');
...
class SubstrateExecutive {
    ...
    async runInference(query) {
        return new Promise((resolve, reject) => {
            try {
                ...
                const attentionPath = join(PROJECT_ROOT, '.ouroboros', 'visualizations', 'real_attention.json');
    ...
    /**
     * Run ratification check on neural paths
     */
    async runRatification() {
        // First ensure neural paths exist (run bridge if needed)
        const pathsExist = existsSync(NEURAL_PATHS);
        
        if (!pathsExist) {
            console.log('  → Generating neural paths...');
            try {
                execSync(`python3 ${join(PROJECT_ROOT, 'bin', 'bridge.py')}`, { encoding: 'utf-8' });
            } catch (e) {
        ...
        // Run ratification
        try {
            execSync(`python3 ${join(PROJECT_ROOT, 'bin', 'ratification.py')}`, { encoding: 'utf-8' });
            const decision = JSON.parse(readFileSync(RATIFIED_DECISION, 'utf-8'));
            return decision;
        } catch (e) {
            console.warn('Ratification failed:', e.message);
            return { ratification: { status: 'error', average_similarity: 0 }, auto_execute: false };
        }
    }
    ...
    async process(query) {
        ...
        // === RATIFICATION CHECK ===
        console.log('\n  Running ratification check...');
        const ratification = await this.runRatification();
        
        if (!ratification.auto_execute) {
            console.log(`  ⚠️  RATIFICATION: ${ratification.ratification.status.toUpperCase()}`);
            console.log(`  📊 Average Similarity: ${(ratification.ratification.average_similarity * 100).toFixed(1)}%`);
            console.log(`  🔴 Paths requiring review: ${ratification.ratification.confidence_breakdown.low_confidence}`);
            
            if (ratification.ratification.status === 'review_required') {
                console.log('\n  🛑 EXECUTION BLOCKED: Human review required');
                console.log('  To approve manually, set thresholds lower or provide explicit approval.');
            ...
            console.log(`  ✅ RATIFICATION: Auto-approved`);
            console.log(`  📊 Average Similarity: ${(ratification.ratification.average_similarity * 100).toFixed(1)}%`);
        }
        // === END RATIFICATION ===
...
Operations:
  ...
  ALLOCATE - Create new tiles
    ...
    if (result.ratification) {
        const rat = result.ratification.ratification;
        console.log(`\n🛡️  Ratification:`);
        console.log(`   Status: ${rat.status.toUpperCase()}`);
        console.log(`   Similarity: ${(rat.average_similarity * 100).toFixed(1)}%`);
        if (rat.status === 'review_required') {
            console.log(`   ⚠️  Blocked: Human review required`);
        }
    }
}
